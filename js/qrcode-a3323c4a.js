...
! function() {
	...
	function a(t, e) {
		for(var r = 1, o = s(t), i = 0, n = c.length; n >= i; i++) {
			var a = 0;
			switch(e) {
				case u.L:
					a = c[i][0];
	...
	t.prototype = {
		...
		setupPositionProbePattern: function(t, e) {
			for(var r = -1; 7 >= r; r++)
				if(!(-1 >= t + r || this.moduleCount <= t + r))
					for(var o = -1; 7 >= o; o++) - 1 >= e + o || this.moduleCount <= e + o || (this.modules[t + r][e + o] = r >= 0 && 6 >= r && (0 == o || 6 == o) || o >= 0 && 6 >= o && (0 == r || 6 == r) || r >= 2 && 4 >= r && o >= 2 && 4 >= o)
		},
			...
			for(var t = f.getPatternPosition(this.typeNumber), e = 0; e < t.length; e++)
				for(var r = 0; r < t.length; r++) {
					...
					if(null == this.modules[o][i])
						for(var n = -2; 2 >= n; n++)
							for(var a = -2; 2 >= a; a++) this.modules[o + n][i + a] = -2 == n || 2 == n || -2 == a || 2 == a || 0 == n && 0 == a
				}
		},
		...
		setupTypeInfo: function(t, e) {
			for(var r = this.errorCorrectLevel << 3 | e, o = f.getBCHTypeInfo(r), i = 0; 15 > i; i++) {
				n = !t && 1 == (1 & o >> i);
				...
				for(6 == a && a--;;) {
					for(var s = 0; 2 > s; s++)
						if(null == this.modules[o][a - s]) {
							var h = !1;
							n < t.length && (h = 1 == (1 & t[n] >>> i)), f.getMask(e, o, a - s) && (h = !h), this.modules[o][a - s] = h, -1 == --i && (n++, i = 7)
						}
		...
		for(var o = 0, i = 0, n = 0, a = new Array(e.length), s = new Array(e.length), h = 0; h < e.length; h++) {
			...
			for(m = 0; m < a[h].length; m++) a[h][m] = 255 & t.buffer[m + o];
			o += u;
			var g = f.getErrorCorrectPolynomial(l),
				d = new r(a[h], g.getLength() - 1).mod(g);
			s[h] = new Array(g.getLength() - 1);
			for(m = 0; m < s[h].length; m++) {
				var c = m + d.getLength() - s[h].length;
				s[h][m] = c >= 0 ? d.get(c) : 0
			}
		}
		for(var p = 0, m = 0; m < e.length; m++) p += e[m].totalCount;
		for(var _ = new Array(p), v = 0, m = 0; i > m; m++)
			for(h = 0; h < e.length; h++) m < a[h].length && (_[v++] = a[h][m]);
		for(m = 0; n > m; m++)
			for(h = 0; h < e.length; h++) m < s[h].length && (_[v++] = s[h][m]);
		return _
	};
	for(var h = {
			MODE_NUMBER: 1,
			...
			getErrorCorrectPolynomial: function(t) {
				for(var e = new r([1], 0), o = 0; t > o; o++) e = e.multiply(new r([1, g.gexp(o)], 0));
				return e
			},
			...
			getLostPoint: function(t) {
				for(var e = t.getModuleCount(), r = 0, o = 0; e > o; o++)
					for(l = 0; e > l; l++) {
						for(var i = 0, n = t.isDark(o, l), a = -1; 1 >= a; a++)
							if(!(0 > o + a || o + a >= e))
								for(var s = -1; 1 >= s; s++) 0 > l + s || l + s >= e || (0 != a || 0 != s) && n == t.isDark(o + a, l + s) && i++;
						i > 5 && (r += 3 + i - 5)
					}
				for(o = 0; e - 1 > o; o++)
					for(l = 0; e - 1 > l; l++) {
						var h = 0;
						t.isDark(o, l) && h++, t.isDark(o + 1, l) && h++, t.isDark(o, l + 1) && h++, t.isDark(o + 1, l + 1) && h++, (0 == h || 4 == h) && (r += 3)
					}
				for(o = 0; e > o; o++)
					for(l = 0; e - 6 > l; l++) t.isDark(o, l) && !t.isDark(o, l + 1) && t.isDark(o, l + 2) && t.isDark(o, l + 3) && t.isDark(o, l + 4) && !t.isDark(o, l + 5) && t.isDark(o, l + 6) && (r += 40);
				for(l = 0; e > l; l++)
					for(o = 0; e - 6 > o; o++) t.isDark(o, l) && !t.isDark(o + 1, l) && t.isDark(o + 2, l) && t.isDark(o + 3, l) && t.isDark(o + 4, l) && !t.isDark(o + 5, l) && t.isDark(o + 6, l) && (r += 40);
				for(var u = 0, l = 0; e > l; l++)
					for(o = 0; e > o; o++) t.isDark(o, l) && u++;
		...
		}, d = 0; 8 > d; d++) g.EXP_TABLE[d] = 1 << d;
	for(d = 8; 256 > d; d++) g.EXP_TABLE[d] = g.EXP_TABLE[d - 4] ^ g.EXP_TABLE[d - 5] ^ g.EXP_TABLE[d - 6] ^ g.EXP_TABLE[d - 8];
	for(d = 0; 255 > d; d++) g.LOG_TABLE[g.EXP_TABLE[d]] = d;
	r.prototype = {
		get: function(t) {
			return this.num[t]
		},
		...
		mod: function(t) {
			...
			for(i = 0; i < t.getLength(); i++) o[i] ^= g.gexp(g.glog(t.get(i)) + e);
		...
		for(var i = r.length / 3, n = [], a = 0; i > a; a++)
			for(var s = r[3 * a + 0], h = r[3 * a + 1], u = r[3 * a + 2], l = 0; s > l; l++) n.push(new o(h, u));
		return n
	...
	var c = [
		...
		m = "svg" === document.documentElement.tagName.toLowerCase() ? p : "undefined" != typeof CanvasRenderingContext2D ? function() {
			function t() {
